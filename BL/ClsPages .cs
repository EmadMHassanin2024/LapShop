...
namespace LapShop.Bl
{
    ...
    public class ClsPages : IPages
    {
        ...
        public ClsPages(LapShop2Context ctx)
        {
            context = ctx;
        }
        public List<TbPages> GetAll()
        {
            try
            {
                var lstCategories = context.TbPage.Where(a => a.CurrentState == 1).ToList();
        ...
        public TbPages GetById(int id)
        {
            try
            {
                var itemType = context.TbPage.FirstOrDefault(a => a.PageId == id && a.CurrentState == 1);
                return itemType;
            }
            catch
            {
